...
function MenuItem({ icon: Icon, label, description, onClick, danger = false }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 p-4 transition-colors rounded-2xl group ${
    ...
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
          danger ? 'bg-red-50 dark:bg-red-900/20 group-hover:bg-red-100 dark:group-hover:bg-red-900/40' : 'bg-[var(--color-background)] group-hover:bg-black/5 dark:group-hover:bg-white/5'
        }`}
      >
        <Icon className={`w-5 h-5 ${danger ? 'text-[var(--color-danger)]' : 'text-[var(--color-text-light)]'}`} />
      </div>
      <div className="flex-1 text-left">
        <p className={`text-sm font-semibold ${danger ? 'text-[var(--color-danger)]' : 'text-[var(--color-text)]'}`}>
          {label}
        </p>
        {description && <p className="text-xs text-[var(--color-muted)] mt-0.5">{description}</p>}
      </div>
      <ChevronRightIcon className={`w-4 h-4 ${danger ? 'text-[var(--color-danger)]' : 'text-[var(--color-muted)]'}`} />
    </button>
  )
}
...
export default function Profile() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="pt-1">
        <h2 className="text-xl font-bold text-[var(--color-text)]">Profil</h2>
      </div>

      <div className="card p-6 flex flex-col items-center text-center">
        <ProfileAvatar name={user?.displayName || user?.id} />
        <h3 className="text-xl font-bold text-[var(--color-text)] mt-4">
          {user?.displayName || user?.id}
        </h3>
        <p className="text-sm text-[var(--color-text-light)] mt-1">@{user?.id}</p>
        {user?.email && (
        ...
        <div className="divide-y divide-[var(--color-border)]">
          <div className="p-2">
            <MenuItem
              ...
              onClick={() => showToast('Pengaturan akun segera hadir', 'info')}
            />
          </div>
          <div className="p-2">
            <MenuItem
              ...
              onClick={() => showToast('Pengaturan keamanan segera hadir', 'info')}
            />
          </div>
          <div className="p-2">
            <MenuItem
              ...
              onClick={() => showToast('Manajemen storage segera hadir', 'info')}
            />
          </div>
          <div className="p-2">
            <MenuItem
          ...
          <div className="bottom-sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center text-center mb-6">
              <div className="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-900/20 flex items-center justify-center mb-4">
                <ArrowRightOnRectangleIcon className="w-7 h-7 text-[var(--color-danger)]" />
              </div>
              <h3 className="text-lg font-bold text-[var(--color-text)] mb-1">Keluar?</h3>
              <p className="text-sm text-[var(--color-text-light)]">
                Kamu akan keluar dari akun <span className="font-semibold text-[var(--color-text)]">{user?.displayName}</span>.
              </p>
            </div>
